...
def run_clustering(ml_ready_df, human_readable_df):
    ...
    reducer = umap.UMAP(n_components=5, n_neighbors=100, min_dist=0.0, random_state=42)
    umap_embeddings = reducer.fit_transform(ml_ready_df)

    # min_cluster_size=500 allows only significant, massive groupings to form clusters (saves API cost!)
    # min_samples=15 makes the algorithm slightly more conservative about what it calls noise
    clusterer = hdbscan.HDBSCAN(min_cluster_size=500, min_samples=15, metric='euclidean')
